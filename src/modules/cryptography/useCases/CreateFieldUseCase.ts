...
export class CreateFieldUseCase {
  ...
  public async execute({ table_id, field_type_name, name }: FieldDto): Promise<Field> {
    ...
    //TO DO ajeitar field tipe
    const field = await this.fieldRepository.create({ table_id, fieldType.id, name });
    return field;
  }
}
