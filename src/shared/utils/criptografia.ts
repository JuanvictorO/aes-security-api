...
export function keyExpansion(keyHex: string) {
  const key = stringToBytes(keyHex);
...
export const subkeysByteToHex = (subkeysArray: Array<Array<number>>) => {
  const chunkSize = 4;
  const result = [];
  for (let i = 0; i < subkeysArray.length; i += chunkSize) {
      const chunk = subkeysArray.slice(i, i + chunkSize);
      result.push(toHexString(chunk[0].concat(chunk[1]).concat(chunk[2]).concat(chunk[3])));
  }
